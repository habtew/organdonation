...
const DoctorProfile: React.FC = () => {
    ...
    const fetchDoctorData = async () => {
      ...
      try {
        ...
        if (userData.role !== UserRole.DOCTOR) {
          // Redirect to correct profile if role mismatches
        ...
        if (doctorDocSnap.exists()) {
          doctorData = doctorDocSnap.data() as Partial<Doctor>;
        }

        // Compose full doctor object for state
        setDoctor({
          id: user.uid,
          name: userData.name,
          ...
          approvals: doctorData.approvals ?? [],
  ...
  if (!doctor) {
    return null; // Redirect handled in effect or no data
  }
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {doctor.approvals.map((approval) => (
                          <tr key={approval.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{approval.donorId}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{approval.receiverId}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{approval.organType}</td>
