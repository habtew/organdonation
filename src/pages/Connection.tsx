...
//   const [connectionRequests, setConnectionRequests] = useState<Record<string, 'pending' | 'accepted' | 'rejected'>>({});
//   const [showConnectModal, setShowConnectModal] = useState(false);
...
//   // Fetch friends and connection requests for currentUser
//   useEffect(() => {
...
//     // Listen to connection requests where currentUser is the sender or receiver
//     const q = query(
//       collection(db, 'connectionRequests'),
//       where('participants', 'array-contains', currentUser.id)
//     );

//     const unsubReqs = onSnapshot(q, (querySnap) => {
//       const reqs: Record<string, 'pending' | 'accepted' | 'rejected'> = {};
//       querySnap.forEach((doc) => {
//         const data = doc.data();
//         const otherUserId = data.fromUserId === currentUser.id ? data.toUserId : data.fromUserId;
//         reqs[otherUserId] = data.status;
//       });
...
//   // Load users based on currentUser.role
//   useEffect(() => {
//     if (!currentUser) return;
...
//         // Fetch user IDs from role-specific collections
...
//       // Determine organs list depending on user role for filter
...
//   // Send connection request: create doc with status 'pending'
//   const sendConnectionRequest = async () => {
//     if (!currentUser || !selectedUser) return;

//     // Check if request already exists
//     if (connectionRequests[selectedUser.id] === 'pending') {
...
//     try {
//       // Find the request doc by participants and status
//       const q = query(
//         collection(db, 'connectionRequests'),
...
//       // Use arrayUnion to append friend IDs safely
//       await updateDoc(currentUserFriendsRef, {
//         [fromUserId]: true,
//       }).catch(() =>
//         setDoc(currentUserFriendsRef, { [fromUserId]: true }, { merge: true })
//       );

//       await updateDoc(fromUserFriendsRef, {
//         [currentUser.id]: true,
//       }).catch(() =>
//         setDoc(fromUserFriendsRef, { [currentUser.id]: true }, { merge: true })
//       );
...
//                   <UserCard key={friend.id} user={friend} showConnectButton={false} />
//                 ))}
//               </div>
//             ) : (
...
//                     showConnectButton={!friends[user.id] && connectionRequests[user.id] !== 'pending'}
//                     pending={connectionRequests[user.id] === 'pending'}
//                     onConnect={() => handleConnect(user)}
//                     onAcceptRequest={
//                       connectionRequests[user.id] === 'pending' &&
//                       // Only show accept if user sent you the request
//                       (!currentUser ? false : user.id !== currentUser.id)
...
//   // Listen to friends and connection requests for currentUser
//   useEffect(() => {
//     if (!currentUser) return;
...
//   // Filter users by name and organ
//   useEffect(() => {
//     if (!users) return;
...
//                   onChat={() => navigateToChat(friend)} // your chat navigation function
//                 />
//                   // <UserCard key={friend.id} user={friend} showConnectButton={false} />
//                 ))}
//               </div>
